...
**动机 / 为什么加分**
现在 `solver_service` 是「按 style 打分取 top-N + 2-opt 调顺序」，只最小化通行时间，营业时间/时间窗都没作为硬约束进优化器。你们引用的 PersTour / OPTW 论文说的正是**带时间窗的定向越野问题**：在时间预算内，从一堆带「奖励值」的点里选一个子集并排序，**最大化总奖励**，同时满足时间窗、营业时间、预算、must-visit。这是能写进技术评审材料、能和精确解比 gap 的真算法升级。
