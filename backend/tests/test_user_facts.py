...
def test_invalidate_facts_keeps_sqlite_row_but_new_sessions_force_refresh(
    tmp_path,
    monkeypatch,
) -> None:
    monkeypatch.setattr("app.agent.store.DB_PATH", tmp_path / "agent_sessions.sqlite", raising=False)
    ...
    from app.agent.user_memory import get_user_facts, invalidate_facts, _read_facts_row

    save_state(_state(session_id="s1", budget=100))
    assert get_user_facts("facts_user", force_refresh=True).session_count == 1

    invalidate_facts("facts_user")
    assert _read_facts_row("facts_user") is not None
...
def test_missing_agent_time_window_uses_typical_user_fact_window(
    tmp_path,
    monkeypatch,
) -> None:
    monkeypatch.setattr("app.agent.store.DB_PATH", tmp_path / "agent_sessions.sqlite", raising=False)
    ...
    save_state(_state(session_id="s1", user_id="time_user", budget=100))

    state = build_initial_state(
        AgentRunRequest(
            user_id="time_user",
            free_text="another route",
            city="hefei",
            date="2026-05-10",
    ...
    assert state.context.time_window.start == "18:00"
    assert state.context.time_window.end == "23:00"
    assert state.profile.time.start_time == "18:00"
    assert state.profile.time.end_time == "23:00"


def test_favorite_districts_uses_structured_poi_district(monkeypatch) -> None:
    from app.agent import user_memory
    ...
    class FakeRepo:
        def get(self, poi_id: str):
            return SimpleNamespace(
                ...
                address="plain address without district token",
            )

    monkeypatch.setattr(user_memory, "get_poi_repository", lambda: FakeRepo())

    districts = user_memory._favorite_districts([SimpleNamespace(stop_poi_ids=["poi_1"])])

    assert districts == ["structured-district"]


def test_poi_detail_exposes_structured_district() -> None:
    ...
    assert poi.district == "structured-district"
...
def test_fact_alignment_uses_structured_district_before_address() -> None:
    from app.schemas.user_memory import UserFacts
    from app.services.poi_scoring_service import PoiScoringService

    facts = UserFacts(
        user_id="facts_user",
        favorite_districts=["structured-district"],
        session_count=3,
        updated_at=datetime.now(timezone.utc),
    )
    poi = SimpleNamespace(
    ...
    score = PoiScoringService(ugc_repo=SimpleNamespace())._fact_alignment_score(poi, facts)
