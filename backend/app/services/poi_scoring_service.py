...
class PoiScoringService:
    def score_poi(
        ...
        *,
        ...
        queue_penalty = -min(poi.queue_estimate["weekend_peak"] / 60 * 12, 12)
    ...
    def _user_interest_score(self, poi, text: str, profile: UserNeedProfile | None) -> float:
        ...
        if profile and profile.party_type and profile.party_type in tags:
            score += 7
        for keyword in ["拍照", "打卡", "photogenic"]:
            if keyword in text and ("photogenic" in tags or "拍照" in tags):
                score += 4
        if any(keyword in text for keyword in ["吃", "饭", "美食", "本地菜"]) and poi.category == "restaurant":
            score += 7
    ...
    def _ugc_match_score(self, poi, text: str) -> float:
        score = 6.0
        keyword_text = " ".join(item["keyword"] for item in poi.high_freq_keywords)
