...
def normalize_category(
    ...
    *,
    ...
    if _contains_any(text, ["咖啡", "冷饮", "甜品", "糕点", "茶餐厅"]):
        return "cafe"
    if _contains_any(text, ["茶艺", "茶馆", "文艺", "展览", "美术馆", "博物馆", "雨天友好"]):
        return "culture"
    if _contains_any(text, ["酒吧", "小酒馆", "夜宵", "夜景"]):
        return "nightlife"
    if _contains_any(text, ["购物", "商场", "商业街", "购物中心", "百货"]):
        return "shopping"
    if _contains_any(text, ["KTV", "电影院", "密室", "桌游", "剧场", "娱乐"]):
        return "entertainment"
    if _contains_any(text, ["公园", "户外", "绿地", "步道", "散步"]):
        return "outdoor"
    if _contains_any(text, ["景点", "风景名胜", "旅游", "寺庙", "教堂", "祠堂", "广场"]):
        return "scenic"
    if _contains_any(text, ["休闲餐饮", "桌游", "剧场", "娱乐"]):
        return "entertainment"
