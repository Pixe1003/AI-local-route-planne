...
from app.repositories.rag_index import ChromaVectorIndex
...
class RetrievalService:
    ...
    def retrieve(self, query: RetrievalQuery) -> list[RetrievedPoi]:
        ...
        for poi_id, group in sorted(grouped.items(), key=lambda item: item[1]["score"], reverse=True):
            ...
            if not within_radius(poi, origin, query.radius_meters):
                continue
            score = float(group["score"] or 0.0)
            results.append(
                RetrievedPoi(
                    poi_id=poi_id,
                    score=round(score, 4),
    ...
    def _query_vector_index(self, text: str, query: RetrievalQuery):
        ...
        try:
            ...
            if "source_types" not in str(exc):
                raise
            return self.vector_index.query(
                text=text,
                city=query.city,
    ...
    def _default_vector_index(self):
        ...
        if not settings.rag_enabled or not settings.embedding_api_key:
            return None
        return ChromaVectorIndex(path=settings.vector_db_path)
